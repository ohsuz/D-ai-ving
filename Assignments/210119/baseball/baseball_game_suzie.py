...
def is_duplicated_number(three_digit):
    ...
    result = (len(three_digit) != len(set(list(three_digit))))
    # ==================================
    return result
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    for user, random in zip(list(user_input_number), list(random_number)):
        if user == random:
            result[0] += 1
...
def main():
    ...
    while True:
        ...
        while not is_real_end:
            user_input2 = input("You win, one more(Y/N)?")
            if user_input2 == '0' or is_no(user_input2):
                is_real_end = True
                break
