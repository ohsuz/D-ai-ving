...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    """
    digit_converter = {'0': 'zero', '1': 'one', '2': 'two', '3': 'three', '4': 'four', '5': 'five', '6': 'six', '7': 'seven', '8': 'eight', '9': 'nine'}
    digit_string = ''
    for c in input_string:
        if c.isdigit():
            digit_string = digit_string + ' ' + digit_converter[c]
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    if '_' not in underscore_str:
        ...
        substrings = underscore_str.split('_')
        camelcase_str = ''.join(s.capitalize() for s in substrings if s)
